...
void get_DeleteData(fstream &);// Delete data Student
...
// Delete data Student
void get_DeleteData(fstream &data){
  ...
  // 1. Pilih nomer data
  cout << "Enter number data with delete: ";
  cin >> nomerData;
  ...
  //    maka kita pindahkan semua data database.bin ke tempSiswa
  dataSementara.open("data.dat", ios::trunc | ios::out | ios::in | ios::binary);
  ...
  data.close();
  data.open("database.bin", ios::trunc | ios::out | ios::binary);
  data.close();
  data.open("database.bin", ios::out | ios::in | ios::binary);
